...
describe('Liking A restaurant', () => {
  ...
  it('should show the like button when the restaurant has not been liked before', async () => {
    ...
    expect(document.querySelector('[aria-label="Tombol Suka"]')).toBeTruthy();
  });

  it('should not show the unlike button when the restaurant has not been liked before', async () => {
    await TestFactories.createLikeButtonPresenterWithRestaurant({ id: 1 });
    expect(document.querySelector('[aria-label="Tombol Tidak Suka"]')).toBeFalsy();
  });
  ...
  it('should not like a restaurant without an id', async () => {
    ...
    expect(console.error).toHaveBeenCalledWith('Restaurant id is not defined or invalid');
  });
});
